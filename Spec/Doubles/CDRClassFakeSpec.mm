...
describe(@"CDRClassFake", ^{
    ...
    describe(@"trying to create a fake for multiple classes", ^{
        it(@"should fail with a reasonable message", ^{
            ^{ nice_fake_for([SimpleIncrementer class], [NSValue class]); } should raise_exception.with_reason(@"Additional interface #1 to fake for class SimpleIncrementer is not a valid protocol");
        });
    });
});
